...
/* Sub-heading inside link col (e.g. KERJASAMA under PARTNER SITE) */
...
.footer-contact-item .fc-icon svg { fill: #a8d5b5; }
.footer-contact-item .fc-text {
    font-size: 12.5px;
    color: rgba(221,229,205,0.75);
    line-height: 1.55;
    font-weight: 500;
}
...
/* Full width border above bottom */
.footer-bottom-wrap {
    ...
    <div class="footer-inner">
        <div class="footer-grid">
            ...
            <div class="footer-col">
                ...
                ?>
                <p><?= !empty($data['slogan']) ? htmlspecialchars($data['slogan']) : 'Belajar Bebas, Berkarya Nyata.' ?></p>
            ...
            if (!empty($menus)):
                // Split menus into 2 columns
                $half    = (int)ceil(count($menus) / 2);
                $col_A   = array_slice($menus, 0, $half);
                $col_B   = array_slice($menus, $half);

                foreach ([$col_A, $col_B] as $col_idx => $col_menus):
                    if (empty($col_menus)) continue;
                    $first = true;
            ?>
            <div class="footer-col">
                <?php foreach ($col_menus as $m):
                    $idmenu = $m['id'];
                    $subs_q = $koneksi_db->sql_query("SELECT * FROM submenu2 WHERE published='1' AND parent='{$idmenu}' ORDER BY ordering ASC");
                    $subs   = [];
                    while ($sub = $koneksi_db->sql_fetchrow($subs_q)) $subs[] = $sub;
                ?>
                    <?php if ($first): ?>
                        <h4><?= htmlspecialchars($m['menu2']) ?></h4>
                    <?php else: ?>
                        <span class="footer-links-subhead"><?= htmlspecialchars($m['menu2']) ?></span>
                    <?php endif; ?>

                    <?php if (!empty($subs)): ?>
                    <ul class="footer-links" <?= !$first ? 'style="margin-bottom:0;"' : '' ?>>
            ...
            <div class="footer-col">
                ...
                <div class="footer-contact-item">
                    ...
                    <div class="fc-text">
                        <?= !empty($data['alamat']) ? htmlspecialchars($data['alamat']) : 'Jl. Ciganitri No 2 Cipagalo Buahahbatu, Bandung' ?>
                    </div>
                </div>
                ...
                <div class="footer-contact-item">
                    ...
                    <div class="fc-text">
                        <?= !empty($data['telp']) ? htmlspecialchars($data['telp']) : '62 811-9081-122' ?>
                    </div>
                </div>
                ...
                <div class="footer-contact-item">
                    ...
                    <div class="fc-text">
                        <?= !empty($data['email']) ? htmlspecialchars($data['email']) : 'info@iaipibandung.ac.id' ?>
                    </div>
                </div>
            </div>
    ...
    <div class="footer-bottom-wrap">
        <div class="footer-bottom">
            <div class="footer-bottom-left">
                &copy; <?= date('Y') ?> MBKM IAI PI Bandung. All rights reserved.
            </div>
            <div>
