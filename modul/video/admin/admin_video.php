<h4>Gallery Video</h4><br/><p>

  <a href="admin.php?pilih=video&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=video&amp;modul=yes&amp;action=add">Add Data</a> |
    <a href="admin.php?pilih=video&amp;modul=yes&amp;action=cari">Cari Data</a>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_video` (`nama`,`ket`,`video`) VALUES ('$nama','$ket','$video')");
	if ($insert) {
...
<td>:</td>
<td>'.input_text ('video',@$_POST['video']).'</td>
</tr>

<tr>
<td></td>
<td></td>
<td>Contoh: https://www.youtube.com/watch?v=<b>V6BpJThrTok</b> <br/>maka hanya dimasukkan <b>V6BpJThrTok</b> </td>
</tr>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_video` SET `nama`='$nama',`ket`='$ket',`video`='$video' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_video` WHERE `id` = '$id'");
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_video` WHERE `id`='$v'");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_video` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
$content .= '<tr>
	<td>'.$no.'</td>
<td>'.$data['nama'].'<br/><b>'.$data['video'].'</b></td>

<td>'.$data['ket'].'</td>


	<td><a href="admin.php?pilih=video&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
<input type="hidden" name="pilih" value="video">
