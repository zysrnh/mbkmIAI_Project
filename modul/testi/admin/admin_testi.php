...
<a href="admin.php?pilih=testi&amp;modul=yes">List Data</a> | 
<a href="admin.php?pilih=testi&amp;modul=yes&amp;action=add">Add Data</a> | 
<a href="admin.php?pilih=testi&amp;modul=yes&amp;action=cari">Cari Data</a>
<?php
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_testi` (`nama`,`ket`,`email`,`tanggal`,`foto`) VALUES ('$nama','$ket','$email','$tanggal','$foto')");
	if ($insert) {
		if (!$image_name){
				...
				$url=str_replace(" ", "-", $nama);
		copy($_FILES['image']['tmp_name'], "./images/testi/".$url.".jpg");
...
createthumbs("images/testi/", "images/testi/thumb/", "$url.jpg", 80);
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_testi` SET `nama`='$nama',`foto`='$foto',`ket`='$ket',`email`='$email' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_testi` WHERE `id` = '$id'");
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_testi`");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_testi` $SORT_SQL LIMIT $offset, $limit");
...
<td>'.$published.'</td>
	<td><a href="admin.php?pilih=testi&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
case 'pub':	
	if ($_GET['pub'] == '0'){	
		$id = int_filter ($_GET['id']);	
		$koneksi_db->sql_query ("UPDATE mod_data_testi SET status='0' WHERE id='$id'");		
	}	
	
	if ($_GET['pub'] == '1'){	
		$id = int_filter ($_GET['id']);	
		$koneksi_db->sql_query ("UPDATE mod_data_testi SET status='1' WHERE id='$id'");			
	}	
	header ("location:admin.php?pilih=testi&modul=yes");
	exit;
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_testi` $query_add");
$jumlah = $koneksi_db->sql_numrows ($num);
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_testi` $query_add $SORT_SQL LIMIT $offset, $limit");
