...
include '../../ikutan/json.php';

if (!isset($_SESSION['modul_ajax'])){
exit;	
}
...
while ($data = $koneksi_db->sql_fetchrow($hasil4)){
///// fungsi hightlight
//////////////////////////////////////////////////////////////////////////////////////////////////////////	
$highlight = $search;
...
$query = $koneksi_db->sql_query ("INSERT INTO `mod_link` (`judul`,`url`,`kid`,`keterangan`,`public`,`date`) VALUES ('$judul','$url','$kategori','$keterangan',0,'".time()."')");
if ($query) {
$open['error'] = false;	
$open['pesanError'] = '';
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_cat_link` ORDER BY `kategori`");
...
$num = $koneksi_db->sql_query("SELECT kid FROM `mod_link` WHERE `kid` = '$kid' AND public = 1");
$jumlah = $koneksi_db->sql_numrows ($num);
///mysql_free_result ($num);	
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_link` WHERE `kid` = '$kid' AND public = 1 ORDER BY `id` DESC LIMIT $offset,$limit");
while ($data = $koneksi_db->sql_fetchrow($query)){
...
$query = $koneksi_db->sql_query ("SELECT `kategori` FROM `mod_cat_link` WHERE `kid` = '$kid'");
$getsql = $koneksi_db->sql_fetchrow($query);
...
$num = $koneksi_db->sql_query("SELECT kid FROM `mod_cat_link`");
$jumlah = $koneksi_db->sql_numrows ($num);
///mysql_free_result ($num);	
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_cat_link` ORDER BY `kid` DESC LIMIT $offset,$limit");
while ($data = $koneksi_db->sql_fetchrow($query)){
$query2 = $koneksi_db->sql_numrows($koneksi_db->sql_query ("SELECT `id` FROM `mod_link` WHERE `kid` = '".$data['kid']."'"));
