...
global $koneksi_db;
$waktu = time () - $expire;
$query1 = $koneksi_db->sql_query("SELECT date FROM $tabel  WHERE date>='$waktu' AND $kategori='$kid' AND public=1");	
	
if ($koneksi_db->sql_numrows ($query1) > 0){
...
global $koneksi_db;
$query = $koneksi_db->sql_query ("SELECT count(`id`) FROM `mod_link` WHERE `public` = 0");
$total = $koneksi_db->sql_fetchrow($query);
$return = '';
if ($total[0] >= 1){
$return = '('.$total[0].')';	
}
...
//| <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=broken'>View Broken Links</a>

$keterangan .="<div><a href='admin.php?pilih=links&amp;modul=yes'>Home</a>  | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=add'>Add Link</a> | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=addkat'>Add Category</a>  | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=incomming'>Incomming Links</a> ".incominglink()." | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=cari'>Cari</a></div>";
...
$num = $koneksi_db->sql_query("SELECT kid FROM ".$GLOBALS['tabel']['cat_link']." ORDER BY kid DESC");
...
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['cat_link']." ORDER BY kid DESC LIMIT $offset, $limit");
...
$no = 0 ;
while ($data = $koneksi_db->sql_fetchrow($result)) {
...
$getcount = $koneksi_db->sql_query("SELECT count(kid) as score FROM ".$GLOBALS['tabel']['link']." WHERE kid='$kid'");
$hasil = $koneksi_db->sql_fetchrow($getcount);

$dl .="<td style=\"width:50%\" valign=\"top\">- <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=lihat&amp;kid=$kid'><b>$kategori</b></a> (".$hasil['score'].") <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=editkat&amp;kid=$kid'>Edit</a>  <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=deletekat&amp;kid=$kid'>Delete</a><br />$keterangans</td>
                           <td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
                            <!-- pisah -->";
   if ($urutan % 2 == 0) {
$dl .= "</tr><tr>";	   
...
for ($i=0; $i<$total_edit; $i++){
	...
	$koneksi_db->sql_query("UPDATE ".$GLOBALS['tabel']['link']." SET 
					 `judul` = '$djuduls',
...
$query = $koneksi_db->sql_query ("SELECT id FROM ".$GLOBALS['tabel']['link']." WHERE kid='".$_POST['kidnya']."'");
...
elseif ($koneksi_db->sql_numrows ($query) > 0){
$ref = $_POST['http_referer'];
$ref = str_replace('&amp;','&',$ref);
header ("location:$ref");
exit;
}else {
header ("location:admin.php?pilih=links&modul=yes");
exit;
}	
...
if (is_array ($explode)){
	foreach ($explode as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE  FROM ".$GLOBALS['tabel']['link']." WHERE id='$v'");
			}
...
$dl .= "<p>&nbsp;</p>Silahkan isi di bawah ini :<form method=\"post\" action=\"admin.php?pilih=links&amp;modul=yes&amp;aksi=mass\">";
for ($i=0; $i<$hitung; $i++){
$id = $_POST['links'][$i];	
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE id='$id'");	
...
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['cat_link']." ORDER BY kategori");
...
if ($hitung > 0){
	...
	<form method="post" action="admin.php?pilih=links&amp;modul=yes&amp;aksi=mass">
	';
	for ($i=0; $i<$hitung; $i++) {
...
$num = $koneksi_db->sql_query("SELECT id FROM ".$GLOBALS['tabel']['link']."  WHERE kid='$kid' ORDER BY id DESC");
$jumlah = $koneksi_db->sql_numrows ($num);
$limit = 10;
$a = new paging ($limit);
...
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE kid='$kid' ORDER BY id DESC  LIMIT $offset, $limit");	
$no = 0 ;
...
$dl .= "<input type=\"checkbox\" name=\"links[]\" value=\"$ID\" style=\"border:none\" /> <b>$JUDUL</b> ($HIT) <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=edit&amp;id=$ID&amp;kid=$KID&amp;pg=$Pg&amp;stg=$Stg&amp;offset=$Offset'>edit</a> | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=delete&amp;id=$ID&amp;kid=$KID' onclick=\"return confirmLink(this, 'DELETE FROM `$ID`  ')\">Hapus</a>
...
$query1 = $koneksi_db->sql_query("UPDATE ".$GLOBALS['tabel']['link']." SET judul='$djudul', keterangan='$dketerangan', url='$durl', kid='$kategori_dl' WHERE id='$id'");
...
$query = $koneksi_db->sql_query("DELETE FROM ".$GLOBALS['tabel']['link']." WHERE id='$id'");
	
if ($query){
//$dl .= "BERhasil di hapus";	
	header ("location:admin.php?pilih=links&amp;modul=yes&aksi=lihat&kid=".$_GET['kid']."");
}
...
$sekarang = time ();	
$hasil = $koneksi_db->sql_query("INSERT INTO ".$GLOBALS['tabel']['link']." (judul, keterangan, url, date, public, kid) 
...
$dl .= "Silahkan isi di bawah ini :<table class='formulir'><tr><td><form method=\"post\" action=\"admin.php?pilih=links&amp;modul=yes&amp;aksi=add\">Judul :<br /><input type='text' name='djudul' size='54' /><br />Url :<br /><input type='text' name='durl' size='54' /><br />";
...
$hasil = $koneksi_db->sql_query("INSERT INTO ".$GLOBALS['tabel']['cat_link']." (kategori, keterangan) 
...
$hasil = $koneksi_db->sql_query("UPDATE ".$GLOBALS['tabel']['cat_link']." SET kategori='$kat_download', keterangan='$ket_download' WHERE kid='$kid'");		
	
$dl .="<br>Berhasil Update kategori <b>$kat_download</b>";	
}	
	
	
$hasil = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['cat_link']." WHERE kid='$kid'");		

$cat = $koneksi_db->sql_fetchrow ($hasil);			
...
$query = $koneksi_db->sql_query("DELETE FROM ".$GLOBALS['tabel']['cat_link']." WHERE kid='$KID'");
	
if ($query){
//$dl .= "BERhasil di hapus";
	
$query = $koneksi_db->sql_query("DELETE FROM ".$GLOBALS['tabel']['link']." WHERE kid='$KID'");
...
$koneksi_db->sql_query("UPDATE ".$GLOBALS['tabel']['link']." SET broken=0 WHERE id='$id'");	
...
$dl .="<br>
    KETERANGAN : <br/>
    <textarea rows=\"7\" name=\"dketerangan[]\" cols=\"40\">$KETERANGAN</textarea>
    <input type=\"hidden\" name=\"id[]\" value=\"$IDs\" />
    ";
//$dl .= '<input type=hidden name="kidnya" value="'.int_filter($_GET['kid']).'">';
}
$ref = $_SERVER['HTTP_REFERER'];
$dl .= "<input type='hidden' name='http_referer' value=\"$ref\"><input type='hidden' name='mode_broken' value='1'><input type=\"submit\" value=\"Edit\" name=submit_edit_form>
    </form>";	
...
$num = $koneksi_db->sql_query("SELECT id FROM ".$GLOBALS['tabel']['link']."  WHERE broken>0 ORDER BY broken DESC");
$jumlah = $koneksi_db->sql_numrows ($num);
$limit = 10;
$a = new paging ($limit);
...
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE broken>0 ORDER BY broken DESC  LIMIT $offset, $limit");	
$no = 0 ;
...
$BROKEN = $row['broken'];
$dl .= '<input type="hidden" name="judul_array[]" value="'.$JUDUL.'">';
$dl .= "<input type=\"checkbox\" name=\"links[]\" value=\"$ID\" style=\"border:none\"> <b>$JUDUL</b> ($HIT) <a href='admin.php?pilih=links&amp;modul=yes&aksi=edit&id=$ID'>edit</a> | <a href='admin.php?pilih=links&amp;modul=yes&aksi=delete&id=$ID'>Hapus</a> | <a href='admin.php?pilih=links&amp;modul=yes&aksi=broken&mode=ya&id=$ID'>Ignore</a>
<br>$KETERANGAN <br>
...
if (!isset ($mode)) $mode = '';
	if ($mode == 'ya'){
$sekarang = time ();
$koneksi_db->sql_query("UPDATE ".$GLOBALS['tabel']['link']." SET public=1, `date`= '$sekarang' WHERE id='$id'");	
...
$num = $koneksi_db->sql_query("SELECT id FROM ".$GLOBALS['tabel']['link']."  WHERE public=0 ORDER BY id DESC");
$jumlah = $koneksi_db->sql_numrows ($num);
$limit = 10;
$a = new paging ($limit);
...
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE public=0 ORDER BY id DESC  LIMIT $offset, $limit");	
$no = 0 ;
...
$dl .= '<input type="hidden" name="judul_array[]" value="'.$JUDUL.'" />';
$dl .= "<input type=\"checkbox\" name=\"links[]\" value=\"$ID\" style=\"border:none\" /> <b>$JUDUL</b> ($HIT) <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=edit&amp;id=$ID'>edit</a> | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=delete&amp;id=$ID'>Hapus</a> | <a href='admin.php?pilih=links&amp;modul=yes&amp;aksi=incomming&amp;mode=ya&amp;id=$ID'>Accept</a>
<br />$KETERANGAN <br />
...
$dl .= "Silahkan isi di bawah ini :<form method=\"post\" action=\"admin.php?pilih=links&amp;modul=yes&amp;aksi=mass\">";
for ($i=0; $i<$hitung; $i++){
$id = $_POST['links'][$i];	
$result = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE id='$id'");	
$row = $koneksi_db->sql_fetchrow($result);
...
$num = $koneksi_db->sql_query("SELECT id,
...
$hasil4 = $koneksi_db->sql_query  ("SELECT judul,
								keterangan,
								url,
								hit,
								date,
...
$was = $koneksi_db->sql_query ("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE $retornar and public=1");

$jumlah = $koneksi_db->sql_numrows ($was);
...
$hasil4 = $koneksi_db->sql_query("SELECT * FROM ".$GLOBALS['tabel']['link']." WHERE $retornar  and public=1 LIMIT $offset, $limit");		
...
$totsearch = $koneksi_db->sql_numrows($hasil4);
if ($totsearch > 0) {
...
while ($row = $koneksi_db->sql_fetchrow ($hasil4)){
///// fungsi hightlight
//////////////////////////////////////////////////////////////////////////////////////////////////////////	
$highlight = $search;
