...
var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
	request.open("GET", 'modul/links/links_data.php?'+querystring, true);	
...
isilinks += '<td valign=top width=50%><img src="modul/links/images/dir3.png" align=absmiddle> <a style="cursor:pointer" onclick="weblink.bukakategori(\'action=detail&id='+auraCMS.ListLink[i].kid+'\')"><b>'+auraCMS.ListLink[i].kategori+'</b></a> ('+auraCMS.ListLink[i].total+') '+newlink+'<br \><span style="font-size:9px">'+auraCMS.ListLink[i].keterangan+'</span></td>';
...
document.getElementById('respon').innerHTML = '<div >Tidak Ada Data</div>';	
}
}
};
		...
		if (request.readyState == 4 && request.responseText != ""){
			...
			if (typeof(auraCMSsimpan) == 'object'){
				...
				Fat.fade_element("responseFade2",null,1000,'#FF3333');
					document.getElementById('responseFade').innerHTML = '<div  id=responseFade2>'+auraCMSsimpan.caption+'</div>';
...
alert('tidak Ada Data');	
		...
		if (auraCMSsimpan.paging != "") html += '<div >'+auraCMSsimpan.paging+'</div>';
...
simpan : function (a) {
	a.disabled = true;
	var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
	request.open("POST", "modul/links/links_data.php?action=add&token="+Math.random(), true);
	request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	request.onreadystatechange = function(){
		if (request.readyState == 4){
			var auraCMSsimpan = eval("(" + request.responseText + ")");
			...
			if (typeof(auraCMSsimpan) == 'object'){
				...
				if (auraCMSsimpan.error == true){
					...
					//<img src="ikutan/code_image.php?math.rand='+Math.random()+'" border="1" alt="Security Code">
					document.getElementById('rahasia').innerHTML = '<img src="ikutan/code_image.php?math.rand='+Math.random()+'" border="1" alt="Security Code">';
					a.disabled = false;
...
var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
request.open("GET", 'modul/links/links_data.php?action=showkategori', true);	
request.onreadystatechange = function(){
var ready = '';	
if (request.readyState == 4){	
var auraCMS = request.responseText != "" ? eval("(" + request.responseText + ")") : null;
...
var html = '<div id="responseFade"></div><blockquote><p><b>Aturan :</b><br>Kode html tidak diijinkan.<br>Catatan : no flood,no spam</p></blockquote><div >';
html +='';
...
html += '</td></tr><tr><td>Keterangan :<br \><textarea name="keterangan" row=20 cols=40></textarea></td></tr><tr><td>Kode : <br \><span id="rahasia"><img src="ikutan/code_image.php?math.rand='+Math.random()+'"></span><br \><input type="text" name="kode" size=10 maxlength="6"></td></tr><tr><td><input type="button" value="add" onclick="weblink.simpan(this)"> <input type="button" value="cancel" onclick="weblink.links()"></td></tr></table></form>';
html += '</div>';
document.getElementById('respon').innerHTML = html;		
},
...
html = '<div >Category: <a style="cursor:pointer;font-weight:bold" onclick="weblink.links()">Home</a> / '+kategorilink+'</div>';
html += '<div >';
html += isilinks;
html += '</div>';
if (auraCMS.paging != "") html += '<div class="border">'+auraCMS.paging+'</div>';
