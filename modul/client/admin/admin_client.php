<h4>Banner Client</h4><br/><p>
<a href="admin.php?pilih=client&amp;modul=yes">List Data</a> | 
<a href="admin.php?pilih=client&amp;modul=yes&amp;action=add">Add Data</a> |
<a href="admin.php?pilih=client&amp;modul=yes&amp;action=cari">Cari Data</a>
...
<input type="hidden" name="pilih" value="client">
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_client` WHERE `id`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_client` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_client` $query_add $SORT_SQL LIMIT $offset, $limit");
...
$content .= '<tr>
	<td>'.$no.'.</td>
		...
		<td><b>'.$data['nama'].'</b><br/>'.$data['link'].'</td>
	<td>'.limitTXT(strip_tags($data['ket']),140).'</td>

	<td><a href="admin.php?pilih=client&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_client` (`nama`,`foto`,`link`,`ket`) VALUES ('$nama','$foto','$link','$ket')");
	if ($insert) {
	...
	$url=str_replace(" ", "-", $nama);
		copy($_FILES['image']['tmp_name'], "./images/client/".$url.".jpg");
}
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_client` SET `nama`='$nama',`foto`='$foto',`link`='$link',`ket`='$ket' WHERE `id` = '$id'");
	if ($insert) {
if (!$image_name){
} else {
	
		copy($_FILES['image']['tmp_name'], "./images/client/".$foto."");
}
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_client` WHERE `id` = '$id'");
