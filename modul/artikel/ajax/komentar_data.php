...
$open['error'] = false;
$open['errorpesan'] = '';
if (!preg_match("/[^a-zA-Z0-9_-]/",$user)) {$error .= "Error: Please enter your name!<br />";}
...
$hasil = $koneksi_db->sql_query ("INSERT INTO komentar (`tgl`,`user`,`email`,`judul`,`konten`,`artikel`,`ip`) VALUES ('$time','$user','$email','$judul','$konten','$id','$IP')");	
if ($hasil){
//fungsi untuk proteksi yang telah memposting  
 posted('komentar_add.php');
}else {
$open['error'] = true;	
$open['errorpesan'] = mysqli_error();
}
...
default:
$id = int_filter($_GET['id']);
$query2 = $koneksi_db->sql_query ("SELECT count(`komentar`.`id`) AS `total` FROM `komentar` WHERE `artikel` = '$id'");
$getdata2 = $koneksi_db->sql_fetchrow($query2);
...
$query = $koneksi_db->sql_query ("SELECT * FROM `komentar` WHERE `artikel` = '$id' ORDER BY `id` DESC LIMIT $offset,$limit");
