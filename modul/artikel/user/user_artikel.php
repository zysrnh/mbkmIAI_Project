...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `artikel` (`judul`,`gambar`,`konten`,`topik`,`tgl`,`user`,`publikasi`,`meta`,`tags`) VALUES ('$judul','$foto','$konten','$topik','$tgl','$user','0','$meta','$tags')");
	if ($insert) {
		if (!$image_name){
...
createthumbs("images/temp/", "images/artikel/", "$foto", 600);
		...
		$content .= '<div class="sukses">Artikel berhasil dimasukkan, menunggu persetujuan admin.</div>';
		posted('alumni');
		unset ($_POST);
		}
	else {
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `artikel`  WHERE publikasi='1' AND topik='$topik' AND user='$user'");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `artikel` WHERE publikasi='1' AND topik='$topik' AND user='$user' $SORT_SQL LIMIT $offset, $limit");
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `artikel`  WHERE publikasi='1' AND user='$user'");
$jumlah = $koneksi_db->sql_numrows ($num);
//mysql_free_result ($num);
...
$query = $koneksi_db->sql_query ("SELECT * FROM `artikel` WHERE publikasi='1' AND user='$user' $SORT_SQL ORDER By `id` DESC  LIMIT $offset, $limit");
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `artikel` $query_add AND publikasi='1' AND user='$user'");
$jumlah = $koneksi_db->sql_numrows ($num);
...
$query = $koneksi_db->sql_query ("SELECT * FROM `artikel` $query_add AND publikasi='1' AND user='$user' $SORT_SQL ORDER By `id` DESC LIMIT $offset, $limit");
