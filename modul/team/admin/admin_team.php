...
<a href="admin.php?pilih=team&amp;modul=yes">List Data</a> | 
<a href="admin.php?pilih=team&amp;modul=yes&amp;action=add">Add Data</a> |
<a href="admin.php?pilih=team&amp;modul=yes&amp;action=cari">Cari Data</a>
...
<input type="hidden" name="pilih" value="team">
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_team` WHERE `id`='$v'");
	}
	}
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_team` $query_add $SORT_SQL LIMIT $offset, $limit");
...
$content .= '<tr>
	...
	<td><a href="admin.php?pilih=team&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_team` (`nama`,`foto`,`pekerjaan`,`hp`,`ket`,`fb`,`tw`,`in`) VALUES ('$nama','$foto','$pekerjaan','$hp','$ket','$fb','$tw','$in')");
	if ($insert) {
	...
	$url=str_replace(" ", "-", $nama);
		copy($_FILES['image']['tmp_name'], "./images/team/".$url.".jpg");
}
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_team` SET `nama`='$nama',`foto`='$foto',`pekerjaan`='$pekerjaan',`hp`='$hp',`ket`='$ket',`fb`='$fb',`tw`='$tw',`in`='$in' WHERE `id` = '$id'");
	if ($insert) {
if (!$image_name){
} else {
	
		copy($_FILES['image']['tmp_name'], "./images/team/".$foto."");
}
