<h4>Topik Artikel</h4><br/><p>

  <a href="admin.php?pilih=topik&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=topik&amp;modul=yes&amp;action=add">Add Data</a> |
    <a href="admin.php?pilih=topik&amp;modul=yes&amp;action=cari">Cari Data</a>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `topik` (`topik`,`ket`) VALUES ('$topik','$ket')");
	if ($insert) {
...
<td>Topik</td>
<td>:</td>
<td>'.input_text ('topik',@$_POST['topik']).'</td>
</tr>
<tr>
<td>Keterangan</td>
<td>:</td>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `topik` SET `topik`='$topik',`ket`='$ket' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `topik` WHERE `id` = '$id'");
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `topik` WHERE `id`='$v'");
			$query2 = $koneksi_db->sql_query ("DELETE FROM `artikel` WHERE `topik`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `topik` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `topik` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
$adatopik=$koneksi_db->sql_numrows($koneksi_db->sql_query("SELECT * FROM artikel where topik='".$id."'"));
$content .= '<tr>
	<td>'.$no.'</td>
<td>'.$data['topik'].'<br/><a href="kategori/'.$data['id'].'/'.$urlxtopik.'.html">kategori/'.$data['id'].'/'.$urlxtopik.'.html</a></td>
<td>'.$data['ket'].'</td>
<td><a href="admin.php?pilih=artikel&modul=yes&action=topik&topik='.$id.'" title="'.$data['topik'].'">'.$adatopik.'</a></td>

	<td><a href="admin.php?pilih=topik&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
$_GET['field'] = !isset ($_GET['field']) ? 'topik' : $_GET['field'];
...
<input type="hidden" name="pilih" value="topik">
