<h4>Pengaturan Slider</h4><br/><p>
<a href="admin.php?pilih=slider&amp;modul=yes">List Data</a> | 
<a href="admin.php?pilih=slider&amp;modul=yes&amp;action=add">Add Data</a> | 
<a href="admin.php?pilih=slider&amp;modul=yes&amp;action=cari">Cari Data</a>
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `slider` (`nama`,`foto`,`ukuran`,`link`,`ket`) VALUES ('$nama','$foto','$ukuran','$link','$ket')");
	if ($insert) {
		if (!$image_name){
				...
				$url=str_replace(" ", "-", $nama);
		copy($_FILES['image']['tmp_name'], "./images/slides/".$url.".jpg");
...
<option value="1920px x 766px" size="19" />1920px x 766px</option>
</select></td>
</tr>
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("UPDATE `slider` SET `nama`='$nama',`foto`='$foto',`ukuran`='$ukuran',`link`='$link',`ket`='$ket' WHERE `id` = '$id'");
	if ($insert) {
		
		if (!$image_name){
} else {
		copy($_FILES['image']['tmp_name'], "./images/slides/".$foto."");
}
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `slider` WHERE `id` = '$id'");
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `slider` WHERE `id`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `slider` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `slider` $query_add $SORT_SQL LIMIT $offset, $limit");
...
$content .= '<tr>
	...
	<td><a href="admin.php?pilih=slider&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
<input type="hidden" name="pilih" value="slider">
