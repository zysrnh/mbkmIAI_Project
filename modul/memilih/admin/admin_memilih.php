<h4>Mengapa Memilih</h4><br/><p>

  <a href="admin.php?pilih=memilih&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=memilih&amp;modul=yes&amp;action=add">Add Data</a> |
    <a href="admin.php?pilih=memilih&amp;modul=yes&amp;action=cari">Cari Data</a>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_memilih` (`nama`,`ket`) VALUES ('$nama','$ket')");
	if ($insert) {
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_memilih` SET `nama`='$nama',`ket`='$ket' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_memilih` WHERE `id` = '$id'");
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_memilih` WHERE `id`='$v'");
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_memilih` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_memilih` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
<td>'.$data['ket'].'</td>
	...
	<td><a href="admin.php?pilih=memilih&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
<input type="hidden" name="pilih" value="memilih">
