...
foreach ($_POST['id'] as $K=>$V){
	...
	if (!empty ($V)) {
		$del = $koneksi_db->sql_query ("DELETE FROM `shoutbox` WHERE `id`='$V'");
	}
...
$pager = $koneksi_db->sql_query ("SELECT id FROM shoutbox");
$jumlah = $koneksi_db->sql_numrows($pager);
...
$content .= "<tr $class><td colspan=\"2\" style=\"padding:10px;\"><input type='checkbox' name='id[]' value='$ID' style='border:0px' /> <a href=\"mailto:$EMAIL\" title=\"Ip: $pecah[0]
$pecah[1]\">" .substr($NAMA,0,15)."</a> : $ISI<br/>$WAKTu</td></tr>\n";
