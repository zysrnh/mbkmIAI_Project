<h4>SEO Meta Pages</h4><br/><p>

  <a href="admin.php?pilih=meta&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=meta&amp;modul=yes&amp;action=add">Add Data</a> |
    <a href="admin.php?pilih=meta&amp;modul=yes&amp;action=cari">Cari Data</a>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_meta` (`nama`,`meta`,`tags`,`judul`) VALUES ('$nama','$meta','$tags','$judul')");
	if ($insert) {
...
<td>Description</td>
<td>:</td>
<td>'.input_textarea2 ('meta',@$_POST['meta']).'</td>
</tr>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_meta` SET `nama`='$nama',`meta`='$meta',`tags`='$tags',`judul`='$judul' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_meta` WHERE `id` = '$id'");
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_meta` WHERE `id`='$v'");
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_meta` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_meta` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
<td>'.$data['tags'].'</td>
	...
	<td><a href="admin.php?pilih=meta&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
<input type="hidden" name="pilih" value="meta">
