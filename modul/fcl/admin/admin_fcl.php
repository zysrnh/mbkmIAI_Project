...
<a href="admin.php?pilih=fcl&amp;modul=yes">List Data</a> | 
<a href="admin.php?pilih=fcl&amp;modul=yes&amp;action=cari">Cari Data</a>
<?php
...
$sort_url_name_img = $_GET['sort'] == 'asc' ? '&nbsp;<IMG height=10 alt=^ src="fcl/_arrowup.gif" width=10 align=absMiddle border=0>' : '&nbsp;<IMG height=10 alt=^ src="fcl/_arrowdown.gif" width=10 align=absMiddle border=0>';
}
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_fcl` WHERE `id`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_fcl`");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_fcl` ORDER By `id` DESC LIMIT $offset, $limit");
...
$published = ($data['status'] == '1') ? '<a href="admin.php?pilih=fcl&modul=yes&amp;action=pub&amp;pub=0&amp;id='.$id.'"><img src="images/tick.gif" border="0" alt="Ya" /></a>' : '<a href="admin.php?pilih=fcl&modul=yes&amp;action=pub&amp;pub=1&amp;id='.$id.'"><img src="images/cross.png" border="0" alt="No" /></a>';	
	

$content .= '<tr>
	<td>'.$no.'.</td>

		<td><img src="images/fcl/'.$data['foto'].'" class="img-responsive" style="max-width:120px;" alt="'.$data['nama'].'"> </td>
	<td><b>'.$data['nama'].'</b><br/>'.$data['email'].'<br/>Telp. '.$data['telp'].'</td>
	<td><b>'.$data['barang'].'</b><br/>'.$data['ket'].'<br/><a href="admin.php?pilih=fcl&amp;modul=yes&amp;action=detail&amp;id='.$id.'">Detail</a></td>
<td>'.$published.'</td>
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_fcl` WHERE `id` = '$id'");
...
<div class="row">
<div class="col-md-6">
<img src="images/fcl/'.$data['foto'].'" class="img-responsive" alt="'.$data['nama'].'">Foto Barang
</div>
<div class="col-md-6">
<img src="images/fcl/'.$data['foto2'].'" class="img-responsive" alt="'.$data['nama'].'">Doc Import
</div>
</div>
...
<br/>
<a href="admin.php?pilih=fcl&modul=yes">Kembali</a>
';
...
case 'pub':	
	if ($_GET['pub'] == '0'){	
		$id = int_filter ($_GET['id']);	
		$koneksi_db->sql_query ("UPDATE mod_data_fcl SET status='0' WHERE id='$id'");		
	}	
	
	if ($_GET['pub'] == '1'){	
		$id = int_filter ($_GET['id']);	
		$koneksi_db->sql_query ("UPDATE mod_data_fcl SET status='1' WHERE id='$id'");			
	}	
	header ("location:admin.php?pilih=fcl&modul=yes");
	exit;
...
<input type="hidden" name="pilih" value="fcl">
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_fcl` $query_add");
$jumlah = $koneksi_db->sql_numrows ($num);
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_fcl` $query_add $SORT_SQL LIMIT $offset, $limit");
